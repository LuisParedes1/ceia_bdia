...
def _session(db: Session, raw: str | None) -> dict:
    ...
    row = db.execute(text("SELECT id,user_id,csrf_hash,tenant_id FROM sessions WHERE token_hash=:hash AND revoked_at IS NULL AND expires_at > now()"), {"hash": _codec().digest(raw)}).mappings().first()
    if not row:
        raise HTTPException(status_code=401, detail="Se requiere autenticación.")
    return dict(row)
...
def _issue_session(db: Session, response: Response, user_id: UUID, tenant_id: UUID) -> None:
    token, csrf = _codec().issue(), _codec().issue()
    expires = _now() + timedelta(minutes=settings.session_ttl_minutes)
    db.execute(text("INSERT INTO sessions (id,user_id,token_hash,csrf_hash,tenant_id,expires_at) VALUES (:id,:user,:token,:csrf,:tenant,:expires)"), {"id": uuid4(), "user": user_id, "token": _codec().digest(token), "csrf": _codec().digest(csrf), "tenant": tenant_id, "expires": expires})
...
def register(payload: RegisterPayload, response: Response, db: Session = Depends(db_session)) -> dict:
    user_id, tenant_id = uuid4(), uuid4()
    role_ids = {name: uuid4() for name in _ROLES}
    try:
        with db.begin():
            db.execute(text("INSERT INTO users (id,email,password_hash) VALUES (:id,:email,:password)"), {"id": user_id, "email": payload.email.lower(), "password": hash_password(payload.password)})
            db.execute(text("SELECT set_config('app.user_id', :value, true)"), {"value": str(user_id)})
            db.execute(text("SELECT set_config('app.tenant_id', :value, true)"), {"value": str(tenant_id)})
            db.execute(text("INSERT INTO tenants (id,name) VALUES (:id,:name)"), {"id": tenant_id, "name": payload.tenant_name})
            db.execute(text("INSERT INTO memberships (tenant_id,user_id) VALUES (:tenant,:user)"), {"tenant": tenant_id, "user": user_id})
            for name, role_id in role_ids.items():
                db.execute(text("INSERT INTO roles (id,tenant_id,name) VALUES (:role,:tenant,:name)"), {"role": role_id, "tenant": tenant_id, "name": name})
            db.execute(text("INSERT INTO permissions (code) VALUES ('members:manage') ON CONFLICT DO NOTHING"))
            db.execute(text("INSERT INTO role_permissions (tenant_id,role_id,permission_code) VALUES (:tenant,:role,'members:manage')"), {"tenant": tenant_id, "role": role_ids["admin"]})
            db.execute(text("INSERT INTO membership_roles (tenant_id,user_id,role_id) VALUES (:tenant,:user,:role)"), {"tenant": tenant_id, "user": user_id, "role": role_ids["admin"]})

            _audit(db, "registration", "success", user_id, tenant_id, str(user_id))
            _issue_session(db, response, user_id, tenant_id)
...
def login(payload: LoginPayload, response: Response, db: Session = Depends(db_session)) -> dict:
    user = db.execute(text("SELECT id,password_hash FROM users WHERE email=:email"), {"email": payload.email.lower()}).mappings().first()
